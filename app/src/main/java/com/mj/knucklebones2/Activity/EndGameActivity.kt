package com.mj.knucklebones2.Activity

import android.content.Intent
import android.os.Bundle
import android.widget.TextView
import androidx.appcompat.app.AppCompatActivity
import com.mj.knucklebones2.R
import com.mj.knucklebones2.databinding.EndGameBinding
...
class EndGameActivity: AppCompatActivity() {
    ...
    fun setEndGameTexts(
        computerName: String,
        computerEndPoints: Int,
        playerName: String,
        playerEndPoints: Int
    ) {
        val winner = if (computerEndPoints > playerEndPoints) computerName else playerName

        endGameHeader.text = String.format(
            getResources().getString(R.string.end_game_header),
            "$winner\nwon the game!"
        )
        endGameText.text = String.format(
            getResources().getString(R.string.end_game_text),
            "$computerName: $computerEndPoints",
            "$playerName: $playerEndPoints"
        )
    }
}
